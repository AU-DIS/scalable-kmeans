...
static void BM_load_data_iris(benchmark::State& state) { 
    //n = 149
    data_file_name = "/mnt/c/Users/kaspe/OneDrive/Skrivebord/Reps/scalable-kmeans/Data/iris.txt";
    //std::cout << data_file_name;
    for (auto _ : state) { 
        runner.init_run(state.range(0),state.range(1),state.range(2),data_file_name);
    }
...
static void BM_load_data_mnist(benchmark::State& state) {
    //n = 60000
    data_file_name = "/mnt/c/Users/kaspe/OneDrive/Skrivebord/Reps/scalable-kmeans/Data/Datagenerators/mnist_28_dct.txt";
    for (auto _ : state) { 
        runner.init_run(state.range(0),state.range(1),state.range(2),data_file_name);
    }
}

static void BM_load_data_mnisttest(benchmark::State& state) {
    //n = 10000
    data_file_name = "/mnt/c/Users/kaspe/OneDrive/Skrivebord/Reps/scalable-kmeans/Data/Datagenerators/mnist_testset_"+std::to_string(int(sqrt(state.range(1))))+"_dct.txt";
    for (auto _ : state) { 
        runner.init_run(state.range(0),state.range(1),state.range(2),data_file_name);
    }
}
...
int main(int argc, char **argv) { 
    ...
    */
    ...
    //BENCHMARK(BM_load_data_flake)->Args({n,d,k})->Repetitions(1)->Iterations(1)->Unit(benchmark::kMillisecond);
    BENCHMARK(BM_load_data_mnisttest)->Args({n,d,k})->Repetitions(1)->Iterations(1)->Unit(benchmark::kMillisecond);
    
    //BENCHMARK(BM_Kmeans_lloyd)->Args({n,d,k})->Unit(benchmark::kMillisecond)->Iterations(1);
    //BENCHMARK(BM_Kmeans_Hamerly)->Args({n,d,k})->Unit(benchmark::kMillisecond)->Iterations(1);
    ...
    //BENCHMARK(BM_Kmeans_gstar)->Args({n,d,k})->Unit(benchmark::kMillisecond)->Iterations(1);
    
    BENCHMARK(BM_Kmeans_Hamerly)->Args({n,d,40})->Unit(benchmark::kMillisecond)->Iterations(1);
    ...
    //BENCHMARK(BM_Kmeans_Hamerly)->Args({n,d,5})->Args({n,d,10})->Args({n,d,15})->Args({n,d,20})->Args({n,d,25})->Args({n,d,30})->Args({n,d,35})->Args({n,d,40})->Unit(benchmark::kMillisecond)->Iterations(1);
